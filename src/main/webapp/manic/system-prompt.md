...
(radius) + `hue` (per-point colour, degrees) + `sat`/`val` (per-point HSL saturation & brightness 0..1 — `val` lets a point go white-hot or dim, so cores glow, grain varies, and a `val=1` ring reads white) optional, other lets are intermediates. ⚠️ A cloud formula is a SANDBOX: it sees ONLY `i`, `t`, `hx`/`hy`/`hz` (from a `from` source), its own lets, `pi`/`tau`/`e`, and functions — the canvas constants `cx`/`cy`/`w`/`h` and scene `parameter`s are NOT in scope (use them in the ctor position/args, but inside the formula hardcode pixel coords, e.g. centre `640,360` on 16:9 / `540,960` on 9:16). Same functions
...
projected by the orbit `camera3` (needs one), with per-point r/hue/sat/val/alpha (from sources are 2D-only) ·
