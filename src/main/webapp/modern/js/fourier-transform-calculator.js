/**
 * Fourier Transform Calculator - DOM/UI logic
 * Computes forward and inverse Fourier transforms via SymPy on OneCompiler.
 * Requires: KaTeX (loaded by JSP)
 * Context path: set window.FT_CALC_CTX before load
 */
(function() {
    'use strict';

    // ========== DOM References ==========
    var forwardInput  = document.getElementById('ft-forward-expr');
    var inverseInput  = document.getElementById('ft-inverse-expr');
    var previewEl     = document.getElementById('ft-preview');
    var computeBtn    = document.getElementById('ft-compute-btn');
    var resultContent = document.getElementById('ft-result-content');
    var resultActions = document.getElementById('ft-result-actions');
    var emptyState    = document.getElementById('ft-empty-state');
    var graphHint     = document.getElementById('ft-graph-hint');
    var forwardWrap   = document.getElementById('ft-forward-wrap');
    var inverseWrap   = document.getElementById('ft-inverse-wrap');
    ...
    function normalizeExpr(expr) {
        ...
        s = s.replace(/\u00b2/g, '^2').replace(/\u00b3/g, '^3')
             ...
             .replace(/\u2212/g, '-')
             .replace(/\u00d7/g, '*');
        var FUNS = 'sin|cos|tan|sec|csc|cot|sinh|cosh|tanh|log|ln|sqrt|Heaviside|DiracDelta|Abs';
    ...
    // ========== Mode Toggle ==========
    var modeBtns = document.querySelectorAll('.ft-mode-btn');
    ...
    // ========== Output Tabs ==========
    var tabBtns = document.querySelectorAll('.ft-output-tab');
    ...
    tabBtns.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            this.classList.add('active');
            document.getElementById('ft-panel-' + panel).classList.add('active');
    ...
    setupToggle('ft-syntax-btn', 'ft-syntax-content');
    setupToggle('ft-pairs-btn', 'ft-pairs-content');

    // ========== Render Fourier Pairs Table ==========
    var pairsData = [
        ['\\delta(t)', '1'],
        ['1', '\\delta(\\omega)'],
        ['e^{-a|t|}', '\\frac{2a}{a^2+4\\pi^2\\omega^2}'],
        ['e^{-at}u(t)', '\\frac{1}{a+2\\pi i\\omega}'],
        ['e^{-\\pi t^2}', 'e^{-\\pi\\omega^2}'],
        ['\\text{rect}(t)', '\\text{sinc}(\\omega)'],
        ['\\text{sinc}(t)', '\\text{rect}(\\omega)'],
        ['\\cos(2\\pi f_0 t)', '\\frac{1}{2}[\\delta(\\omega-f_0)+\\delta(\\omega+f_0)]'],
        ['te^{-at}u(t)', '\\frac{1}{(a+2\\pi i\\omega)^2}']
    ];
    for (var i = 0; i < pairsData.length; i++) {
        try {
            katex.render(pairsData[i][0], document.getElementById('ft-pair-f' + i), { throwOnError: false });
            katex.render(pairsData[i][1], document.getElementById('ft-pair-F' + i), { throwOnError: false });
        } catch(e) {}
    }

    // ========== Quick Examples ==========
    var forwardExamples = [
        { label: 'e^(-t\u00B2)', expr: 'exp(-t^2)' },
        { label: 'e^(-|t|)', expr: 'exp(-Abs(t))' },
        { label: 'rect(t)', expr: 'Heaviside(t+1/2)-Heaviside(t-1/2)' },
        { label: 'e^(-t)u(t)', expr: 'exp(-t)*Heaviside(t)' },
        { label: 'sinc(t)', expr: 'sin(pi*t)/(pi*t)' },
        { label: 'te^(-t)u(t)', expr: 't*exp(-t)*Heaviside(t)' },
        { label: 'e^(-t\u00B2/2)', expr: 'exp(-t^2/2)' },
        { label: '\u03B4(t)', expr: 'DiracDelta(t)' },
        { label: 'cos(2\u03C0t)', expr: 'cos(2*pi*t)' },
        { label: 'e^(-3t)u(t)', expr: 'exp(-3*t)*Heaviside(t)' }
    ];
    var inverseExamples = [
        { label: '1/(1+w\u00B2)', expr: '1/(1+w^2)' },
        { label: '\u03B4(w)', expr: 'DiracDelta(w)' },
        { label: 'e^(-\u03C0w\u00B2)', expr: 'exp(-pi*w^2)' },
        { label: '2/(4\u03C0\u00B2w\u00B2+1)', expr: '2/(4*pi^2*w^2+1)' },
        { label: '1/(2i\u03C0w+1)', expr: '1/(2*I*pi*w+1)' },
        { label: 'rect(w)', expr: 'Heaviside(w+1/2)-Heaviside(w-1/2)' },
        { label: 'e^(-|w|)', expr: 'exp(-Abs(w))' },
        { label: '1/(1+4\u03C0\u00B2w\u00B2)', expr: '1/(1+4*pi^2*w^2)' }
    ];

    var randomForward = [
        { expr: 'exp(-t^2)' }, { expr: 'exp(-Abs(t))' },
        { expr: 'Heaviside(t+1/2)-Heaviside(t-1/2)' },
        { expr: 'exp(-t)*Heaviside(t)' }, { expr: 'sin(pi*t)/(pi*t)' },
        { expr: 't*exp(-t)*Heaviside(t)' }, { expr: 'exp(-t^2/2)' },
        { expr: 'DiracDelta(t)' }, { expr: 'cos(2*pi*t)' },
        { expr: 'exp(-3*t)*Heaviside(t)' }, { expr: 'exp(-2*t)*Heaviside(t)' },
        { expr: 't^2*exp(-t^2)' }, { expr: 'cos(4*pi*t)' },
        { expr: 'exp(-Abs(t))*cos(2*pi*t)' }, { expr: 'exp(-t^2)*sin(pi*t)' },
        { expr: 't*exp(-t^2)' }
    ];
    var randomInverse = [
        { expr: '1/(1+w^2)' }, { expr: 'DiracDelta(w)' },
        { expr: 'exp(-pi*w^2)' }, { expr: '2/(4*pi^2*w^2+1)' },
        { expr: '1/(2*I*pi*w+1)' }, { expr: 'Heaviside(w+1/2)-Heaviside(w-1/2)' },
        { expr: 'exp(-Abs(w))' }, { expr: '1/(1+4*pi^2*w^2)' },
        { expr: '1/(1+9*pi^2*w^2)' }, { expr: 'exp(-2*pi*Abs(w))' },
        { expr: '1/(4+w^2)' }, { expr: 'exp(-w^2)' }
    ];

    // ========== Random Button ==========
    document.getElementById('ft-random-btn').addEventListener('click', function() {
    ...
    function updateExamples() {
        var container = document.getElementById('ft-examples');
        var examples = currentMode === 'forward' ? forwardExamples : inverseExamples;
        var html = '';
        for (var i = 0; i < examples.length; i++) {
            html += '<button type="button" class="ft-example-chip" data-expr="' + escapeHtml(examples[i].expr) + '">' + escapeHtml(examples[i].label) + '</button>';
        }
        container.innerHTML = html;
    }
    updateExamples();

    document.getElementById('ft-examples').addEventListener('click', function(e) {
        var chip = e.target.closest('.ft-example-chip');
        if (!chip) return;
    ...
    function updatePreview() {
        try {
            ...
            if (currentMode === 'forward') {
                ...
                latex = '\\mathcal{F}\\left\\{' + exprToLatex(expr) + '\\right\\}';
            } else {
                var expr = normalizeExpr(inverseInput.value.trim());
                if (!expr) {
                    previewEl.innerHTML = '<span style="color:var(--text-muted);font-size:0.8125rem;">Type a function of \u03C9 above\u2026</span>';
                    return;
                }
                latex = '\\mathcal{F}^{-1}\\left\\{' + exprToLatex(expr) + '\\right\\}';
            }
    ...
    function buildSympyCode(mode) {
        var code = 'from sympy import symbols, fourier_transform, inverse_fourier_transform, simplify, latex, sin, cos, tan, exp, log, sqrt, pi, sinh, cosh, tanh, Heaviside, DiracDelta, Abs, I, oo\n';
        code += 'from sympy import Function, Rational\n';
        code += 'import json, numpy as np\n';
        code += 't = symbols("t", real=True)\n';
        code += 'w = symbols("w", real=True)\n';
        code += 'a = symbols("a", positive=True)\n\n';

        if (mode === 'forward') {
            var expr = exprToPython(normalizeExpr(forwardInput.value.trim()));
            code += 'f_expr_str = "' + expr.replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '"\n';
            code += 'f = eval(f_expr_str, {"t": t, "w": w, "sin": sin, "cos": cos, "tan": tan, "exp": exp, "log": log, "sqrt": sqrt, "pi": pi, "sinh": sinh, "cosh": cosh, "tanh": tanh, "Heaviside": Heaviside, "DiracDelta": DiracDelta, "Abs": Abs, "I": I})\n\n';
            code += 'try:\n';
            code += '    F = fourier_transform(f, t, w)\n';
            code += '    F = simplify(F)\n';
            ...
            code += 'steps.append({"title": "Definition of Fourier transform", "latex": r"\\\\mathcal{F}\\\\{f(t)\\\\} = \\\\int_{-\\\\infty}^{\\\\infty} f(t) \\\\, e^{-2\\\\pi i \\\\omega t} \\\\, dt"})\n';
            code += 'steps.append({"title": "Identify transform type", "latex": "\\\\text{Apply standard Fourier pair or theorem}"})\n';
            code += 'steps.append({"title": "Apply Fourier transform", "latex": "F(\\\\omega) = " + latex(F)})\n';
            code += 'steps.append({"title": "Final result", "latex": r"\\\\boxed{\\\\mathcal{F}\\\\{" + latex(f) + r"\\\\} = " + latex(F) + r"}"})\n';
            ...
            code += '    t_vals = np.linspace(-5, 5, 200)\n';
            ...
            code += 'F_raw = eval("' + expr.replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '", {"w": w, "t": t, "sin": sin, "cos": cos, "tan": tan, "exp": exp, "log": log, "sqrt": sqrt, "pi": pi, "sinh": sinh, "cosh": cosh, "tanh": tanh, "Heaviside": Heaviside, "DiracDelta": DiracDelta, "Abs": Abs, "I": I})\n\n';
            code += 'try:\n';
            code += '    f = inverse_fourier_transform(F_raw, w, t)\n';
            code += '    f = simplify(f)\n';
            code += 'except Exception as e:\n';
            code += '    print("ERROR:" + str(e))\n';
            code += '    import sys; sys.exit(0)\n\n';
            ...
            // Steps
            code += 'steps = []\n';
            code += 'steps.append({"title": "Given F(\\u03C9)", "latex": "F(\\\\omega) = " + latex(F_raw)})\n';
            code += 'steps.append({"title": "Definition of inverse Fourier transform", "latex": r"\\\\mathcal{F}^{-1}\\\\{F(\\\\omega)\\\\} = \\\\int_{-\\\\infty}^{\\\\infty} F(\\\\omega) \\\\, e^{2\\\\pi i \\\\omega t} \\\\, d\\\\omega"})\n';
            code += 'steps.append({"title": "Apply inverse Fourier transform", "latex": r"f(t) = " + latex(f)})\n';
            code += 'steps.append({"title": "Final result", "latex": r"\\\\boxed{\\\\mathcal{F}^{-1}\\\\{" + latex(F_raw) + r"\\\\} = " + latex(f) + r"}"})\n';
            code += 'print("STEPS:" + json.dumps(steps))\n\n';
    ...
    function doCompute() {
        ...
        var modeLabel = currentMode === 'forward' ? 'forward Fourier transform' : 'inverse Fourier transform';
        resultContent.innerHTML = '<div style="text-align:center;padding:2rem;">' +
            '<div class="ft-spinner" style="width:24px;height:24px;border-width:3px;margin:0 auto 1rem;"></div>' +
        ...
        fetch((window.FT_CALC_CTX || '') + '/OneCompilerFunctionality?action=execute', {
            method: 'POST',
    ...
    function showResult(mode, resultLatex, resultText, steps) {
        ...
        var modeLabel = mode === 'forward' ? 'Forward Fourier Transform' : 'Inverse Fourier Transform';
        var html = '<div class="ft-result-math">';
        html += '<div class="ft-result-label">' + modeLabel + '</div>';
        html += '<div class="ft-result-main" id="ft-r-result"></div>';

        html += '<div class="ft-result-detail"><span class="ft-method-badge">' + modeLabel + ' (SymPy CAS)</span></div>';
        html += '<button type="button" class="ft-steps-btn" id="ft-steps-btn">&#128221; Show Steps</button>';
        html += '<div id="ft-steps-area"></div>';
        html += '</div>';
        resultContent.innerHTML = html;

        katex.render(lastResultLatex, document.getElementById('ft-r-result'), { displayMode: true, throwOnError: false });

        var stepsBtn = document.getElementById('ft-steps-btn');
        if (stepsBtn) {
    ...
    function renderSteps(steps) {
        var container = document.getElementById('ft-steps-area');
        ...
        var html = '<div class="ft-steps-container">';
        html += '<div class="ft-steps-header">';
        ...
        html += '<span class="ft-steps-sympy-badge">CAS</span>';
        html += '</div>';

        for (var i = 0; i < steps.length; i++) {
            html += '<div class="ft-step">';
            html += '<span class="ft-step-num">' + (i + 1) + '</span>';
            html += '<div class="ft-step-body">';
            html += '<div class="ft-step-title">' + escapeHtml(steps[i].title) + '</div>';
            html += '<div class="ft-step-math" id="ft-step-math-' + i + '"></div>';
            html += '</div></div>';
        }
        html += '</div>';
        container.innerHTML = html;
        ...
        for (var j = 0; j < steps.length; j++) {
            var el = document.getElementById('ft-step-math-' + j);
            if (el && steps[j].latex) {
                try {
    ...
    function showError(msg) {
        ...
        html += '<li>Use ^ for powers: t^2 or (w+1)^2</li>';
        html += '<li>For forward mode: expression must be in terms of t</li>';
        html += '<li>For inverse mode: expression must be in terms of w</li>';
        html += '</ul>';
        html += '</div>';
        resultContent.innerHTML = html;
        if (emptyState) emptyState.style.display = 'none';
    ...
    function renderGraph(cfg) {
        ...
        var container = document.getElementById('ft-graph-container');
    ...
    function buildCompilerCode() {
        ...
        if (currentMode === 'forward') {
            var expr = exprToPython(normalizeExpr(forwardInput.value.trim())) || 'exp(-t**2)';
            code += '# Forward Fourier Transform\nf = ' + expr + '\n\n';
            code += 'F = fourier_transform(f, t, w)\n';
            code += 'F = simplify(F)\n\n';
            code += 'print(f"f(t) = {f}")\n';
            code += 'print(f"F{{f(t)}} = F(w) = {F}")\n';
        } else {
            var expr = exprToPython(normalizeExpr(inverseInput.value.trim())) || '1/(1+w**2)';
            code += '# Inverse Fourier Transform\nF = ' + expr + '\n\n';
            code += 'f = inverse_fourier_transform(F, w, t)\n';
            code += 'f = simplify(f)\n';
            code += 'print(f"F(w) = {F}")\n';
            code += 'print(f"F^(-1){{F(w)}} = f(t) = {f}")\n';
        }
        return code;
    }
    ...
    function loadCompilerWithTemplate() {
        ...
        var iframe = document.getElementById('ft-compiler-iframe');
        iframe.src = (window.FT_CALC_CTX || '') + '/onecompiler-embed.jsp?c=' + encodeURIComponent(config);
    }

    // ========== Copy / Share ==========
    document.getElementById('ft-copy-latex-btn').addEventListener('click', function() {
    ...
    // ========== Download PDF ==========
    document.getElementById('ft-download-pdf-btn').addEventListener('click', function() {
        downloadResultPdf();
    });
    ...
    function downloadResultPdf() {
        ...
        title.textContent = 'Fourier Transform Calculator \u2014 8gwifi.org';
        container.appendChild(title);

        var divider = document.createElement('div');
        divider.style.cssText = 'height:2px;background:linear-gradient(90deg,#7c3aed,#a855f7,transparent);margin-bottom:24px;';
        ...
        try {
            ...
            var previewLatex = currentMode === 'forward'
                ? '\\mathcal{F}\\left\\{' + exprToLatex(inputExpr) + '\\right\\}'
                : '\\mathcal{F}^{-1}\\left\\{' + exprToLatex(inputExpr) + '\\right\\}';
        ...
        aMath.style.cssText = 'font-size:22px;margin-bottom:16px;padding:16px;background:#f5f3ff;border-radius:8px;';
        ...
        var stepsArea = document.getElementById('ft-steps-area');
        if (stepsArea && stepsArea.children.length > 0) {
            ...
            var stepEls = stepsArea.querySelectorAll('.ft-step');
            for (var i = 0; i < stepEls.length; i++) {
                var stepRow = document.createElement('div');
                stepRow.style.cssText = 'display:flex;gap:12px;margin-bottom:12px;';

                var stepNum = document.createElement('div');
                stepNum.style.cssText = 'width:24px;height:24px;background:#7c3aed;color:#fff;border-radius:50%;font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;';
                ...
                var titleEl = stepEls[i].querySelector('.ft-step-title');
                if (titleEl) {
                ...
                var mathEl = stepEls[i].querySelector('.ft-step-math');
                if (mathEl) {
        ...
        footer.innerHTML = '<span>Generated by 8gwifi.org Fourier Transform Calculator</span><span>' + new Date().toLocaleDateString() + '</span>';
        ...
        loadHtml2Canvas.then(function() {
            ...
            pdf.save('fourier-transform-' + currentMode + '.pdf');
    ...
    // ========== Print Worksheet ==========
    document.getElementById('ft-worksheet-btn').addEventListener('click', function() {
        generateWorksheet();
    });
    ...
    function generateWorksheet() {
        ...
        html += '<title>Fourier Transform Worksheet</title>';
        ...
        html += '.ws-section-number { display: inline-block; width: 28px; height: 28px; background: #7c3aed; color: #fff; text-align: center; line-height: 28px; font-size: 14px; font-weight: bold; margin-right: 8px; }';
        ...
        html += '<button onclick="window.print()" style="padding:10px 28px;font-size:15px;font-weight:600;background:#7c3aed;color:#fff;border:none;border-radius:6px;cursor:pointer;">Print Worksheet</button>';
        html += '</div>';

        html += '<div class="ws-header">';
        html += '<div class="ws-title">Fourier Transform Worksheet</div>';
        ...
        // Inverse section
        html += '<div class="ws-section">';
        html += '<div class="ws-section-title"><span class="ws-section-number">II</span>Inverse Fourier Transform &mdash; Find f(t)</div>';
        for (var i = 0; i < invProblems.length; i++) {
            html += '<div class="ws-problem">';
            html += '<span class="ws-problem-num">' + problemNum + '.</span>';
            html += 'Compute <strong>F<sup>&minus;1</sup>{F(&omega;)}</strong> where ';
            html += '<span class="ws-problem-expr" data-katex="F(\\omega) = ' + texEscape(invProblems[i].expr) + '"></span>';
            html += '<div class="ws-workspace"></div>';
        ...
        html += '<div class="ws-footer">Generated by 8gwifi.org Fourier Transform Calculator &mdash; ' + new Date().toLocaleDateString() + '</div>';
    ...
    document.getElementById('ft-share-btn').addEventListener('click', function() {
        ...
        if (typeof ToolUtils !== 'undefined') {
            var url = ToolUtils.generateShareUrl(params, { toolName: 'Fourier Transform Calculator' });
    ...
    try {
        ...
        if (shareMode && (shareMode === 'forward' || shareMode === 'inverse')) {
            var btn = document.querySelector('.ft-mode-btn[data-mode="' + shareMode + '"]');
            if (btn) btn.click();
