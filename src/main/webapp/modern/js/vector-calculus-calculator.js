...
(function() {
    ...
    function generateWorksheet() {
        // Pick 4 problems per mode (12 total), shuffled
        var gradProblems = shuffleArray(randomGradient).slice(0, 4);
        var divProblems  = shuffleArray(randomDivergence).slice(0, 4);
        var curlProblems = shuffleArray(randomCurl).slice(0, 4);
        ...
        html += '<title>Vector Calculus Worksheet</title>';
        ...
        html += '.ws-instructions { font-size: 13px; font-style: italic; margin-bottom: 20px; padding: 8px 12px; background: #f9f9f9; border-left: 3px solid #333; }';
        ...
        html += '.ws-section-number { display: inline-block; width: 28px; height: 28px; background: #333; color: #fff; text-align: center; line-height: 28px; font-size: 14px; font-weight: bold; margin-right: 8px; }';
        ...
        html += '.ws-answer-key { page-break-before: always; }';
        html += '.ws-answer-key .ws-ak-title { font-size: 18px; font-weight: bold; text-align: center; margin-bottom: 16px; border-bottom: 2px solid #333; padding-bottom: 8px; }';
        html += '.ws-ak-item { margin-bottom: 8px; font-size: 14px; }';
        html += '.ws-ak-item .ws-ak-num { font-weight: bold; margin-right: 4px; }';
        html += '.ws-ak-note { font-size: 11px; color: #666; font-style: italic; text-align: center; margin-top: 16px; }';
        html += '.ws-footer { text-align: center; font-size: 10px; color: #999; margin-top: 24px; border-top: 1px solid #ddd; padding-top: 8px; }';
        html += '@media print { body { padding: 0; } .ws-no-print { display: none; } .ws-workspace { min-height: 110px; } }';
        html += '</style></head><body>';

        // Print button (no-print)
        html += '<div class="ws-no-print" style="text-align:center;margin-bottom:16px;">';
        html += '<button onclick="window.print()" style="padding:10px 28px;font-size:15px;font-weight:600;background:#4f46e5;color:#fff;border:none;border-radius:6px;cursor:pointer;">Print Worksheet</button>';
        html += ' <button onclick="window.print()" style="padding:10px 28px;font-size:15px;font-weight:600;background:#fff;color:#4f46e5;border:2px solid #4f46e5;border-radius:6px;cursor:pointer;margin-left:8px;">Save as PDF</button>';
        html += '</div>';

        // Header
        html += '<div class="ws-header">';
        html += '<div class="ws-title">Vector Calculus Worksheet</div>';
        html += '<div class="ws-subtitle">Gradient, Divergence &amp; Curl</div>';
        html += '</div>';
        ...
        // Instructions
        html += '<div class="ws-instructions">';
        html += '<strong>Instructions:</strong> Compute the indicated quantity for each problem. Show all partial derivatives and simplify your final answer. Write your answer on the line provided.';
        html += '</div>';
        ...
        html += '<div class="ws-section-title"><span class="ws-section-number">I</span>Gradient &mdash; Find &#8711;f</div>';
        for (var i = 0; i < gradProblems.length; i++) {
            var expr = gradProblems[i].scalar;
            html += '<div class="ws-problem">';
            html += '<span class="ws-problem-num">' + problemNum + '.</span>';
            html += 'Compute <strong>&#8711;f</strong> where &nbsp;';
            html += '<span class="ws-problem-expr" data-katex="f(x,y,z) = ' + texEscape(expr) + '"></span>';
            html += '<div class="ws-workspace"></div>';
            html += '<div class="ws-answer-line">Answer: &#8711;f = </div>';
            html += '</div>';
            answerKeyData.push({ num: problemNum, mode: 'gradient', expr: expr });
            problemNum++;
        }
        html += '</div>';

        // ---- Section II: Divergence ----
        html += '<div class="ws-section">';
        html += '<div class="ws-section-title"><span class="ws-section-number">II</span>Divergence &mdash; Find &#8711; &middot; <b>F</b></div>';
        for (var i = 0; i < divProblems.length; i++) {
            var p = divProblems[i];
            html += '<div class="ws-problem">';
            html += '<span class="ws-problem-num">' + problemNum + '.</span>';
            html += 'Compute <strong>&#8711; &middot; F</strong> where &nbsp;';
            html += '<span class="ws-problem-expr" data-katex="\\mathbf{F} = \\langle ' + texEscape(p.fx) + ',\\; ' + texEscape(p.fy) + ',\\; ' + texEscape(p.fz) + ' \\rangle"></span>';
            html += '<div class="ws-workspace"></div>';
            html += '<div class="ws-answer-line">Answer: &#8711; &middot; F = </div>';
            html += '</div>';
            answerKeyData.push({ num: problemNum, mode: 'divergence', fx: p.fx, fy: p.fy, fz: p.fz });
            problemNum++;
        }
        html += '</div>';
        ...
        // ---- Section III: Curl ----
        html += '<div class="ws-section">';
        html += '<div class="ws-section-title"><span class="ws-section-number">III</span>Curl &mdash; Find &#8711; &times; <b>F</b></div>';
        for (var i = 0; i < curlProblems.length; i++) {
            var p = curlProblems[i];
            html += '<div class="ws-problem">';
            html += '<span class="ws-problem-num">' + problemNum + '.</span>';
            html += 'Compute <strong>&#8711; &times; F</strong> where &nbsp;';
            html += '<span class="ws-problem-expr" data-katex="\\mathbf{F} = \\langle ' + texEscape(p.fx) + ',\\; ' + texEscape(p.fy) + ',\\; ' + texEscape(p.fz) + ' \\rangle"></span>';
            html += '<div class="ws-workspace"></div>';
            html += '<div class="ws-answer-line">Answer: &#8711; &times; F = </div>';
            html += '</div>';
            answerKeyData.push({ num: problemNum, mode: 'curl', fx: p.fx, fy: p.fy, fz: p.fz });
            problemNum++;
        }
        html += '</div>';

        // ---- Answer Key (separate page) ----
        html += '<div class="ws-answer-key">';
        html += '<div class="ws-ak-title">Answer Key</div>';
        html += '<div class="ws-ak-note">Answers generated via SymPy. Use the calculator at 8gwifi.org to verify step-by-step solutions.</div><br>';

        for (var i = 0; i < answerKeyData.length; i++) {
            var a = answerKeyData[i];
            html += '<div class="ws-ak-item">';
            html += '<span class="ws-ak-num">' + a.num + '.</span> ';
            if (a.mode === 'gradient') {
                html += '<span data-katex-answer="gradient" data-expr="' + escapeHtml(a.expr) + '">Computing...</span>';
            } else if (a.mode === 'divergence') {
                html += '<span data-katex-answer="divergence" data-fx="' + escapeHtml(a.fx) + '" data-fy="' + escapeHtml(a.fy) + '" data-fz="' + escapeHtml(a.fz) + '">Computing...</span>';
            } else {
                html += '<span data-katex-answer="curl" data-fx="' + escapeHtml(a.fx) + '" data-fy="' + escapeHtml(a.fy) + '" data-fz="' + escapeHtml(a.fz) + '">Computing...</span>';
            }
            html += '</div>';
        }
        html += '</div>';
        ...
        // Footer
        html += '<div class="ws-footer">Generated by 8gwifi.org Vector Calculus Calculator &mdash; ' + new Date().toLocaleDateString() + '</div>';

        // KaTeX rendering script + answer computation
        html += '<script src="' + katexJS + '"><\/script>';
        html += '<script>';
        html += 'document.addEventListener("DOMContentLoaded", function() {';
        // Render all problem expressions with KaTeX
        ...
        html += '  }';
        // Compute answers via SymPy for answer key
        html += '  var answers = document.querySelectorAll("[data-katex-answer]");';
        html += '  var answerQueue = [];';
        html += '  for (var i = 0; i < answers.length; i++) answerQueue.push(answers[i]);';
        html += '  function processNext() {';
        html += '    if (answerQueue.length === 0) return;';
        html += '    var el = answerQueue.shift();';
        html += '    var mode = el.getAttribute("data-katex-answer");';
        html += '    var code = "from sympy import symbols, diff, simplify, latex\\n";';
        html += '    code += "x, y, z = symbols(\'x y z\')\\n";';
        html += '    if (mode === "gradient") {';
        html += '      var expr = el.getAttribute("data-expr");';
        html += '      code += "f = " + toPy(expr) + "\\n";';
        html += '      code += "gx = simplify(diff(f,x))\\n";';
        html += '      code += "gy = simplify(diff(f,y))\\n";';
        html += '      code += "gz = simplify(diff(f,z))\\n";';
        html += '      code += "print(\'RESULT:\' + latex(gx) + \' , \' + latex(gy) + \' , \' + latex(gz))\\n";';
        html += '    } else if (mode === "divergence") {';
        html += '      code += "Fx = " + toPy(el.getAttribute("data-fx")) + "\\n";';
        html += '      code += "Fy = " + toPy(el.getAttribute("data-fy")) + "\\n";';
        html += '      code += "Fz = " + toPy(el.getAttribute("data-fz")) + "\\n";';
        html += '      code += "d = simplify(diff(Fx,x) + diff(Fy,y) + diff(Fz,z))\\n";';
        html += '      code += "print(\'RESULT:\' + latex(d))\\n";';
        html += '    } else {';
        html += '      code += "Fx = " + toPy(el.getAttribute("data-fx")) + "\\n";';
        html += '      code += "Fy = " + toPy(el.getAttribute("data-fy")) + "\\n";';
        html += '      code += "Fz = " + toPy(el.getAttribute("data-fz")) + "\\n";';
        html += '      code += "cx = simplify(diff(Fz,y) - diff(Fy,z))\\n";';
        html += '      code += "cy = simplify(diff(Fx,z) - diff(Fz,x))\\n";';
        html += '      code += "cz = simplify(diff(Fy,x) - diff(Fx,y))\\n";';
        html += '      code += "print(\'RESULT:\' + latex(cx) + \' , \' + latex(cy) + \' , \' + latex(cz))\\n";';
        html += '    }';
        html += '    fetch("' + (window.VC_CALC_CTX || '') + '/OneCompilerFunctionality?action=execute", {';
        html += '      method: "POST",';
        html += '      headers: {"Content-Type": "application/x-www-form-urlencoded"},';
        html += '      body: "language=python&code=" + encodeURIComponent(code)';
        html += '    }).then(function(r) { return r.json(); })';
        html += '    .then(function(data) {';
        html += '      var out = (data.stdout || data.output || "");';
        html += '      var m = out.match(/RESULT:(.+)/);';
        html += '      if (m) {';
        html += '        var tex = m[1].trim().replace(/\\\\\\\\/g, "\\\\");';
        html += '        if (mode === "gradient" || mode === "curl") {';
        html += '          var parts = tex.split(" , ");';
        html += '          var symbol = mode === "gradient" ? "\\\\nabla f" : "\\\\nabla \\\\times \\\\mathbf{F}";';
        html += '          var full = symbol + " = \\\\langle " + parts.join(",\\\\; ") + " \\\\rangle";';
        html += '          try { katex.render(full, el, { displayMode: false, throwOnError: false }); }';
        html += '          catch(e) { el.textContent = tex; }';
        html += '        } else {';
        html += '          try { katex.render("\\\\nabla \\\\cdot \\\\mathbf{F} = " + tex, el, { displayMode: false, throwOnError: false }); }';
        html += '          catch(e) { el.textContent = tex; }';
        html += '        }';
        html += '      } else { el.textContent = "See calculator for solution"; }';
        html += '      setTimeout(processNext, 200);';
        html += '    }).catch(function() { el.textContent = "See calculator for solution"; setTimeout(processNext, 200); });';
        html += '  }';
        html += '  processNext();';
        html += '});';
        html += 'function toPy(s) {';
        html += '  if (!s) return "0";';
        html += '  s = s.replace(/\\^/g, "**");';
        html += '  s = s.replace(/e\\*\\*\\(([^)]+)\\)/g, "exp($1)");';
        html += '  s = s.replace(/e\\*\\*([a-zA-Z0-9]+)/g, "exp($1)");';
        html += '  s = s.replace(/(\\d)([a-zA-Z])/g, "$1*$2");';
        html += '  s = s.replace(/\\bln\\(/g, "log(");';
        html += '  return s;';
        html += '}';
        html += '<\/script>';
        html += '</body></html>';
    ...
    function texEscape(s) {
        // Minimal transform of user expression to LaTeX-safe for KaTeX data attribute
        // Helper: find matching close-paren for an open-paren at position i
        ...
        // Replace sqrt(...) → \sqrt{...} with balanced parens
        ...
        // Replace trig/log fn(...) → \fn(...) (keep parens, just add backslash)
        ...
        s = s.replace(/\^\(([^)]+)\)/g, '^{$1}');
        // sqrt → \sqrt{...} (brace-wrapped)
        s = replaceFnBrace(s, 'sqrt', '\\sqrt');
        // trig/log → \trig( ... ) (backslash prefix, keep parens)
        var fns = ['sin','cos','tan','sec','csc','cot','sinh','cosh','tanh','log','ln','arcsin','arccos','arctan'];
