/**
 * 2×2 SVG net renderer.  Same pattern as rubiks/cube-net.js but on the
 * 8×6 grid.  Mounts once, returns an updater that swaps fills + highlights
 * in place.
 */

import { FACE_COLORS, FACES, GRID_COLS, GRID_ROWS, PLACEMENTS } from './cube.js';

const STICKER_SIZE = 56;   // bigger than 3×3 since there are fewer stickers
const STICKER_GAP  = 3;
const RADIUS       = 6;
...
export function mountCubeNet(host, opts = {}) {
    ...
    svg.setAttribute('aria-label', "Pocket cube unfolded net");
    svg.style.maxWidth = `${width}px`;
    svg.style.display  = 'block';

    const rects = new Array(24);
    ...
    function paint() {
        ...
        for (const p of PLACEMENTS) {
            ...
            r.setAttribute('stroke-width', isHi ? 4 : 1.5);
