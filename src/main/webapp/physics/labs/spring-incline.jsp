...
<jsp:include page="../../modern/components/seo-tool-page.jsp">
    <jsp:param name="toolName" value="Spring on Inclined Plane — Does Angle Change the Period?" />
    <jsp:param name="toolCategory" value="Physics" />
    <jsp:param name="toolDescription" value="A mass on a spring slides on a ramp. See how the ramp angle shifts the equilibrium but does NOT change the oscillation period. Add friction to see energy dissipation. Free, no signup." />
    <jsp:param name="toolUrl" value="physics/labs/spring-incline.jsp" />
    <jsp:param name="toolKeywords" value="spring inclined plane, oscillation on ramp, SHM incline, spring mass ramp, friction spring, period angle independent, AP physics spring" />
    ...
    <jsp:param name="teaches" value="spring on incline, equilibrium shift, period independence, friction, normal force, kinetic friction, SHM" />
    <jsp:param name="faq1q" value="Does the angle of an inclined plane affect the period of a spring oscillator?" />
    <jsp:param name="faq1a" value="No. The period T = 2 pi sqrt(m/k) is the same at every angle. The angle only changes the equilibrium position by shifting it downhill by mg sin theta divided by k. The restoring force from the spring has the same spring constant regardless of angle." />
    <jsp:param name="faq2q" value="How does friction affect a spring on an inclined plane?" />
    <jsp:param name="faq2a" value="Kinetic friction f = mu times mg cos theta opposes the motion and dissipates energy. The oscillation amplitude decreases over time. If static friction is strong enough the mass may stop before reaching the equilibrium position." />
</jsp:include>
...
<div class="ad-lab-hero" id="ad_lab_hero"><div class="ad-label">Advertisement</div></div>
  <nav class="lab-crumb">
    ...
    <span>Spring on Incline</span>
  </nav>
  <h1 class="lab-title">Spring on Inclined Plane</h1>
  <div id="labTabs"></div>
  <div class="lab-grid">
...
import { SpringInclineSim } from './js/sims/spring-incline.js';
const lab = createLab(SpringInclineSim, {
...
document.getElementById('labInfo').innerHTML = SpringInclineSim.info || '';
</script>
<script>
(function(){
