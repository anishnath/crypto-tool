...
<jsp:include page="../../modern/components/seo-tool-page.jsp">
    <jsp:param name="toolName" value="Pulley-Spring Scale System — Why Doesn't the Scale Read 200N?" />
    <jsp:param name="toolCategory" value="Physics" />
    <jsp:param name="toolDescription" value="Two weights hang from a rope over two pulleys with a spring scale in the middle. Does the scale read 100N or 200N? Explore this classic tension misconception with an interactive simulation. Free, no signup." />
    <jsp:param name="toolUrl" value="physics/labs/pulley-scale.jsp" />
    <jsp:param name="toolKeywords" value="pulley spring scale, tension in rope, Atwood machine, physics misconception, Newton's laws, free body diagram, pulley system, spring scale reading, tension force" />
    ...
    <jsp:param name="teaches" value="tension, pulleys, Atwood machine, Newton's second law, free body diagrams, spring scale, force measurement, misconceptions" />
    <jsp:param name="faq1q" value="What does a spring scale read when two equal weights are hung from a pulley system?" />
    <jsp:param name="faq1a" value="The spring scale reads the tension in the rope, which equals the weight of one mass (mg), not the sum of both weights. For two 100N weights the scale reads 100N not 200N." />
    <jsp:param name="faq2q" value="Why doesn't the scale read 200N with two 100N weights?" />
    <jsp:param name="faq2a" value="Because tension in a continuous frictionless rope is constant throughout. The scale measures the tension which is the same force that supports each weight individually. It is like standing on a bathroom scale — the scale reads your weight not your weight plus the floor's reaction force." />
</jsp:include>
...
<div class="ad-lab-hero" id="ad_lab_hero"><div class="ad-label">Advertisement</div></div>
  ...
  <nav class="lab-crumb">
    ...
    <span>Pulley-Spring Scale</span>
  </nav>

  <h1 class="lab-title">Pulley–Spring Scale System</h1>

  <div class="misconception-callout" style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:8px 16px;margin:8px 0;font:500 14px/1.4 'DM Sans',sans-serif;color:var(--lab-text);">
    <strong style="color:#EF4444;">Question:</strong> Two 100 N weights hang from a rope over pulleys. A spring scale is in the middle. Does the scale read <strong>100 N</strong> or <strong>200 N</strong>?
  </div>
...
import { PulleyScaleSim } from './js/sims/pulley-scale.js';

const lab = createLab(PulleyScaleSim, {
...
document.getElementById('labInfo').innerHTML = PulleyScaleSim.info || '';
</script>
