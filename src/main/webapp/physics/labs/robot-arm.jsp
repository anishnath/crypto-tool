...
<jsp:include page="../../modern/components/seo-tool-page.jsp">
    <jsp:param name="toolName" value="Two-Link Robot Arm — Inverse Kinematics Simulator" />
    <jsp:param name="toolCategory" value="Physics" />
    <jsp:param name="toolDescription" value="Simulate a two-link robot arm with forward and inverse kinematics. Drag the end effector, visualize the Jacobian, workspace boundaries, and singularities. Circle and figure-8 trajectory modes. Free, no signup." />
    <jsp:param name="toolUrl" value="physics/labs/robot-arm.jsp" />
    <jsp:param name="toolKeywords" value="robot arm, inverse kinematics, forward kinematics, two link, Jacobian, singularity, workspace, end effector, joint angles, robotics, SCARA" />
    <jsp:param name="toolImage" value="logo.png" />
    <jsp:param name="breadcrumbCategoryUrl" value="physics/" />
    <jsp:param name="educationalLevel" value="Undergraduate, Graduate, Robotics, Mechanical Engineering" />
    <jsp:param name="teaches" value="forward kinematics, inverse kinematics, Jacobian matrix, singularity, workspace analysis, velocity control, elbow up elbow down solutions" />
    <jsp:param name="faq1q" value="What is inverse kinematics and why does a two-link arm have two solutions?" />
    <jsp:param name="faq1a" value="Inverse kinematics computes the joint angles needed to place the end effector at a desired position. A two-link arm has two solutions because the elbow can bend either way to reach the same point. These are called elbow-up and elbow-down configurations. The two solutions come from the plus or minus sign when computing theta 2 from the cosine inverse." />
    <jsp:param name="faq2q" value="What is a singularity in robot kinematics and when does it occur?" />
    <jsp:param name="faq2a" value="A singularity occurs when the Jacobian matrix becomes singular meaning its determinant is zero. For a two-link arm the determinant equals L1 times L2 times sine of theta 2 which is zero when the arm is fully extended theta 2 equals 0 or fully folded theta 2 equals pi. At a singularity the arm cannot move in certain directions and the inverse Jacobian blows up causing infinite joint velocities." />
</jsp:include>
...
<div class="ad-lab-hero" id="ad_lab_hero"><div class="ad-label">Advertisement</div></div>
  ...
  <h1 class="lab-title">Two-Link Robot Arm (Forward & Inverse Kinematics)</h1>
  <div id="labTabs"></div>
  <div class="lab-grid">
  ...
  <div class="lab-related">
    ...
    <a href="<%=request.getContextPath()%>/physics/labs/cart-pole.jsp">Cart-Pole</a>
...
import { RobotArmSim } from './js/sims/robot-arm.js';
const lab = createLab(RobotArmSim, {
...
document.getElementById('labInfo').innerHTML = RobotArmSim.info || '';
</script>
<script>
(function(){
