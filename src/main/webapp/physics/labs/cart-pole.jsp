...
<jsp:include page="../../modern/components/seo-tool-page.jsp">
    <jsp:param name="toolName" value="Cart-Pole Simulator — Inverted Pendulum with LQR Control" />
    <jsp:param name="toolCategory" value="Physics" />
    <jsp:param name="toolDescription" value="Simulate the cart-pole inverted pendulum system. Compare open-loop instability with LQR state-feedback control. Adjust mass, length, inertia, friction. Drag the pole to disturb it. Free, no signup." />
    <jsp:param name="toolUrl" value="physics/labs/cart-pole.jsp" />
    <jsp:param name="toolKeywords" value="cart pole, inverted pendulum, LQR control, state feedback, control theory, reinforcement learning, unstable equilibrium, mass matrix, Newton Euler, benchmark control" />
    <jsp:param name="toolImage" value="logo.png" />
    <jsp:param name="breadcrumbCategoryUrl" value="physics/" />
    <jsp:param name="educationalLevel" value="Undergraduate, Graduate, Control Systems, Robotics" />
    <jsp:param name="teaches" value="inverted pendulum dynamics, LQR control, state-space representation, mass matrix form, unstable equilibrium, linearization, eigenvalue analysis" />
    <jsp:param name="faq1q" value="Why is the inverted pendulum unstable and how does LQR control stabilize it?" />
    <jsp:param name="faq1a" value="The inverted pendulum is unstable because the linearized system has a positive eigenvalue meaning any small disturbance grows exponentially. The LQR controller applies a horizontal force F on the cart that is proportional to the state vector F equals negative K times theta theta dot x x dot. The gain vector K is chosen to minimize a quadratic cost balancing control effort against state deviation making all closed loop eigenvalues negative and the system stable." />
    <jsp:param name="faq2q" value="What are the equations of motion for the cart-pole system?" />
    <jsp:param name="faq2a" value="The cart-pole equations come from Newton second law applied to the coupled system. The mass matrix form is M plus m times x double dot minus m l cosine theta times theta double dot equals F minus b x dot minus m l sine theta times theta dot squared. The second equation is negative m l cosine theta times x double dot plus I plus m l squared times theta double dot equals m g l sine theta. Inverting the mass matrix gives explicit formulas for x double dot and theta double dot." />
</jsp:include>
...
<div class="ad-lab-hero" id="ad_lab_hero"><div class="ad-label">Advertisement</div></div>
  ...
  <h1 class="lab-title">Cart-Pole (Inverted Pendulum with LQR Control)</h1>
  <div id="labTabs"></div>
  <div class="lab-grid">
  ...
  <div class="lab-related">
    ...
    <a href="<%=request.getContextPath()%>/physics/labs/cart-pendulum.jsp">Cart + Pendulum</a>
    <a href="<%=request.getContextPath()%>/physics/labs/quadrotor.jsp">Quadrotor</a>
...
import { CartPoleSim } from './js/sims/cart-pole.js';
const lab = createLab(CartPoleSim, {
...
document.getElementById('labInfo').innerHTML = CartPoleSim.info || '';
</script>
<script>
(function(){
